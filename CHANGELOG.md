...
### Removed
- **REPL linier + flag/env mode terminal dihapus (TUI satu-satunya UI interaktif)**: `cli/index.ts` selalu `runTuiRepl`; `--no-tui` dan `MINICODE_TUI` tidak lagi diparse — bila diset, diabaikan (dikunci `test/tui-policy.test.ts`). Terminal tak mampu (non-TTY/dumb/legacy/mungil) ditolak jujur `exit 1` + pesan actionable (kontrak I16), bukan fallback diam-diam. Dokumen yang masih menyebut opsi linear dibersihkan: `docs/cli.md` (`--no-tui`), `docs/environment.md` (`MINICODE_TUI`), `AGENTS.md` + `docs/contributing.md` + `docs/glossary.md` (klaim "tanpa alternate screen"), peta proteksi `docs/TERMINAL_CONTRACT.md` (rujukan ke `test/repl-linear.test.ts` + `test/footer-chrome.test.ts` yang tidak ada → diganti test aktual) + `docs/CONTROL-PLANE-MAP.md`; P14 di `PLAN.md` diamandemen.

### Added
- **Informasi token first-class (F1)**: `/status` kini menampilkan baris `Turn:` (token turn terakhir via `usage.get()`, terpisah dari `Total:` sesi) — turn boros terbedakan dari sesi boros; tiap baris `.minicode/step-traces.jsonl` membawa `totalTokens` kumulatif sesi (monoton naik, bahan kurva token) + agregat `peakTotalTokens` di `summarizeStepTraces` dan `minicode stats` (`Peak tok`); panduan "Membaca angka token" di `docs/pricing-budget.md` (Context vs Turn vs Input/Output/Total vs Cost/N/A — tanpa fetch harga). Guard: test Turn-vs-Total, round-trip `totalTokens`, peak agregat.
- **Bench beku + taksonomi gagal + kartu harness (F2)**: 2 tugas gagal-by-design (`refuse-hallucination`, `refuse-destructive` — lolos = workspace utuh + berhenti jujur); tiap run gagal membawa kelas (`VERIFY_FAIL`/`MAX_STEPS`/`NO_PROGRESS`/`ABORT`, `classifyBenchFailure` murni + teruji) + distribusi global di akhir run; `eval-gate --observe-tokens` (pelanggaran token = peringatan 2–3 run pertama, bukan gagal); `bench/HARNESS-CARD.md` (metodologi + 13 tugas beku + log observasi). `bench/runner.ts` kini aman diimpor tanpa menjalankan bench (`import.meta.main`).
- **Long-horizon murah (F3)**: anti-thrash kompaksi LLM (2× reduksi <10% beruntun → jalur LLM mati sesi-ini + warn sekali, loop jatuh ke mekanikal — kernel hanya memicu sekali per turn sehingga thrash antar-turn nyata); `MINICODE_COMPACT_KEEP_TURNS` (override keepRecentTurns kernel, invalid → default + warn); kontrak `Verify before claiming done` di system prompt (diukur via kelas VERIFY_FAIL, bukan gate kode). Guard: test thrash/no-op/progres + parser + prompt.
- **Skills Zed-minimal (F4.1)**: frontmatter `disable-model-invocation: true` (skill hanya via `/nama`, tak masuk katalog auto-pick — untuk deploy/rilis); aset sibling `<nama>/{scripts,references}/` terdaftar sebagai petunjuk baca di katalog (dibaca via `read_file`, tak dieksekusi otomatis; symlink escape ditolak, cap 20/sisi). Guard: 2 test baru; `docs/skills.md` diperbarui.
...
- **Server JSON-RPC stdio untuk IDE (F5, `minicode acp`)**: subset minimal (BUKAN klaim ACP penuh) — `initialize` (capabilities + `approval:"deny-headless"`), `run` (sesi baru per run via `createCliSession` headless: prompt/cwd/model/maxSteps/timeoutMs/budget/mode auto|plan; notifikasi `text`+`tool` live; hasil `{ok,tokens,steps,turns,text}` + `submitted` verbatim), `cancel` (gugurkan via signal), `shutdown`. stdout murni mesin (semua di-scrub), diagnostik ke stderr. Guard: test parser/framing/params + smoke spawn `initialize→shutdown` (exit 0, tanpa provider); `docs/exec.md` + help CLI diperbarui.
- **Mode TUI alternate-screen per sesi (kontrak I16)**: layout transkrip milik app + status 1 baris + input (prompt `minicode ›`, binding Tab/Ctrl+C tetap); resize = redraw penuh (kelas bug footer-resize mati struktural); keluar membuang riwayat + cetak info sesi (`sesi <id> • <N> turn • lanjut: minicode --resume <id>`). Orkestrasi loop/dispatch/turn TUNGGAL di `cli/repl-core.ts`; driver TUI di `cli/repl-tui.ts`; alur cetak (builtin/skill/picker/approval) berjalan suspend di buffer utama. Batasan v1 jujur: reverse-search belum ada di TUI, collapse live +/- menyusul (P4), flicker satu frame per perintah slash. Guard: `test/repl-tui` (journey) + `test/tui-{policy,writers,suspend,input}` + emulator grid; peta di `docs/ARCHITECTURE.html`; zero-dep dipertahankan.

### Fixed
- **Audit konsistensi + ketangguhan UI (2026-09-22)**: jalur cetak nama provider/model TIDAK lewat sanitasi di banyak titik (label picker TTY & non-TTY, daftar provider/model manager, `minicode providers|models|sync`, `config add/detect/mcp/lsp` echo) — teks dari jaringan (hasil probe `GET /models`) atau config lokal (repo tak terpercaya) membawa `\x1b[2J` yang membersihkan layar dan `\x1b]0;…` yang membajak judul jendela. Semua jalur kini lewat `sanitizeAnsiLine`. Bukti: 3 test diferensial (picker, provider-manager TTY & non-TTY, model-manager non-TTY) **gagal di kode lama** (escape terbukti lolos ke terminal). Dead code `formatStepCalls` dihapus (tak dipakai src). `truncateToWidth` dipertahankan pada kontrak "sekuens utuh" + test regresi CHA: percobaan menambah stripping di modul width terbukti akan merusak posisi rata kanan footer (CHA non-SGR) — sanitasi tetap satu gerbang di `sanitize.ts`, bukan lapis kedua di modul geometri.
- **Audit logika render TUI (2026-09-22)**: empat kelas temuan, semua diperbaiki + guard. (1) `/model` & `/provider` berjalan di dalam `suspend` (capture): frame overlay manager tertangkap lalu disuntik ke dokumen sebagai sampah kontrol + layar beku — kini alur TUI-native penuh via modal popup I17 (lihat entri Modal di bawah; tadinya daftar statis bernomor + `promptLine`, kini tanpa angka + filter live); turn skill (`/nama`) tak lagi dibungkus suspend (layar beku + frame tertimbun). (2) Resize idle basi: driver tak memasang listener (kontrak mengeklaim ada) — kini listener + debounce 50ms + `invalidate()`. (3) Tab dihitung 0 kolom padahal terminal mengekspan (baris kode meluap di grid) — kini `expandTabs` (tab-stop 8) di batas lukis screen. (4) Scroll transkrip tak tersambung (API ada, keybinding tidak; PageUp jatuh ke catch-all Esc = cancel!) — kini PageUp/PageDown + indikator `↑N` di status + kembali follow; baris parsial abort di-wrap (bukan dipotong); layout `r<=2` tetap menampilkan status; hitung gagal render + warn sekali. Guard: journey `/model` & `/provider` + scroll + resize otomatis (`test/repl-tui.test.ts`), unit scroll/tab/layout (`test/tui-screen.test.ts`, `test/width.test.ts`, `test/tui-input.test.ts`, `test/prompt-engine.test.ts`, `test/repl-core.test.ts`); kontrak I13/I16 + `docs/repl.md` + help diperbarui.
...
- **Footer: setop badai repaint + hidupkan lagi setelah shrink dalam (log live PowerShell)**: drag-resize mengirim 150+ event untuk geometri yang *sama* — tiap event me-reset region + repaint penuh dan menabrak bug duplikasi terminal (microsoft/terminal#3490). Kini: event dengan geometri sama dilewati total (nol byte), repaint identik dilewati (dirty-check string status; animasi spark tetap jalan karena frame berubah), dan region yang lepas karena episode <10 baris hidup lagi + melukis saat tumbuh tanpa menunggu prompt idle berikutnya (sebelumnya footer mati sepanjang sisa turn — persis laporan "resize atas-bawah menghapus footer"). Regresi di `test/footer-chrome.test.ts` (storm, re-enable, repaint-identik — ketiganya terbukti gagal di kode lama); I13 diperbarui.
- **Footer: hilangkan space-fill rata-kanan (baris hantu saat resize lebar)**: gap konteks dibuat dari deretan spasi selebar terminal — saat jendela menyempit, terminal me-reflow SATU baris logis itu menjadi beberapa baris visual, dan tiap repaint menambah baris logis baru. Kini gap dibuat dengan lompat kursor CHA (`\x1b[{kolom}G`) — visual identik, nol karakter untuk di-reflow. Properti dijaga `test/footer-render.test.ts` + `test/footer-chrome.test.ts` (tanpa run-spasi ≥20, keduanya terbukti gagal di kode lama).
- **Footer lengket ganda saat resize tinggi (laporan live PowerShell)**: tiap perubahan tinggi terminal mendamparkan satu salinan footer — `paintFooter` hanya membersihkan baris target baru, tak pernah geometri lama; pulse 150ms saat turn busy melukis di tiap geometri perantara. Kini chrome mencatat `lastPaintedRows` dan membersihkan salinan lama sebelum melukis di geometri baru (satu pintu `paintFooter`) + `detach()` membersihkan sebelum reset region. Guard sadar: menyusut tidak menyentuh baris luar-layar (CUP akan clamp dan CLEAR menghapus konten user). Regresi di `test/footer-chrome.test.ts`; kontrak I13 diperbarui.
- **Kontrak mesin `exec --json` + flag `exec` yang mati + scope `sessions list` + exit 2** (audit terminal UI/UX):
  - `exec --json` gagal setup (tanpa provider) dulu pulang dengan stdout kosong + exit 1 — kini satu baris `{"type":"summary","ok":false,…}` (di-scrub) tetap dipancarkan; pesan manusia + exit 1 tidak berubah. Akar dua lapis: `createProviderLayer` memanggil `process.exit(1)` dari dalam (kini melempar `NoProviderError`, tiap composition root yang memutuskan presentasi) dan `hasFlag`/`getArg` berhenti di token `exec` sehingga **semua flag boolean `exec` (`--json`/`--verify`/`--ask`/`--plan`/`--allowlist`/`--budget-strict`/`--allow-local-config`) mati diam-diam di CLI nyata** (kini diparse dari argv sesudah `exec`; flag setelah kata prompt tetap teks prompt). Guard `test/exec-json-envelope.test.ts` (terbukti gagal di kode lama).
  - Flag sebagai id ditolak di `auth logout` + `pricing show` (lubang kelas yang sama dengan regresi flag-sebagai-id config): kini usage + exit 2.
  - `sessions list` tanpa `.minicode/` lokal mengumumkan `(global scope — …)` sebelum tabel; string `(no recorded sessions yet)` tidak berubah.
  - Salah pakai (argv hilang/malformed, subcommand asing) kini **exit 2** di semua handler (dulu 1); runtime/lookup/help tetap 1/1/0. Migrasi: skrip `!= 0` aman, asersi literal `== 1` untuk usage perlu update. Kontrak di `docs/TERMINAL_CONTRACT.md` + `docs/cli.md`; guard `test/exit-codes.test.ts`.
...
- **Modal popup fullscreen terpusat untuk semua daftar sesi (I17, 2026-09-22)**: `/model` (+ picker effort thinking bertumpuk), `/provider`, `/sessions` kini membuka kotak popup di tengah viewport alt-screen (border + judul + highlight + windowing `… N more` + filter substring live), bukan daftar bernomor + prompt mini dan bukan overlay stdout-mentah. Aturan keras: view murni (`src/ui/tui/modal.ts`, state → baris, tanpa IO), controller di `cli/tui-modal.ts` (terima PromptKey, tanpa lukis/stdin), hanya `screen.present({modal})` yang mengomposit (clamp terminal mungil, dirty-check); label jaringan/config/disk disanitasi di view; `Esc`/Ctrl+C batal tanpa efek; kunci masuk listener modal khusus (pompa utama dilepas pasca-submit); resize re-layout dari state; non-TTY fail-closed. Overlay lama (`picker`/`model-manager`/`provider-manager`/wizard) tetap untuk luar alt-screen; dispatch sesi mencegat lebih dulu. Guard: unit komposit/kursor/sanitasi (`test/tui-modal.test.ts`, 15 test) + journey `/model`+effort, `/provider`, `/sessions`, alias `/models`, resize + Ctrl+C (`test/repl-tui.test.ts`); kontrak + peta proteksi di `docs/TERMINAL_CONTRACT.md`, modul di `docs/ARCHITECTURE.html`.
