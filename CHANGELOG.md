...
### Added
- **Mode TUI alternate-screen per sesi (slice pertama yang bisa dijalankan, kontrak I16)**: `MINICODE_TUI=auto|always|never` + `--no-tui` (auto = TUI bila TTY+mampu, linear bila tidak; non-TTY/dumb/legacy/pendek selalu linear). Layout transkrip milik app + status 1 baris + input (prompt `minicode ›`, binding Tab/Ctrl+C tetap); resize = redraw penuh (kelas bug footer-resize mati struktural); keluar membuang riwayat + cetak info sesi (`sesi <id> • <N> turn • lanjut: minicode --resume <id>`). Orkestrasi loop/dispatch/turn TUNGGAL di `cli/repl-core.ts` (ekstrak dari `repl.ts` tanpa ubah perilaku linier — dijaga suite lama); driver TUI di `cli/repl-tui.ts`; alur cetak (builtin/skill/picker/approval) berjalan suspend di buffer utama. Batasan v1 jujur: reverse-search belum ada di TUI, collapse live +/- menyusul (P4), flicker satu frame per perintah slash. Guard: `test/repl-tui` (journey) + `test/tui-{policy,writers,suspend,input}` + emulator grid; peta di `docs/ARCHITECTURE.html`; zero-dep dipertahankan.
