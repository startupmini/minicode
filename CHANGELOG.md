...
### Fixed — Task system durability & evidence-gated completion

- Task state sekarang bertahan lintas `--resume`. `cli/index.ts` menurunkan `sessionId` dari `--session` atau sebuah id acak, **tidak** dari `--resume`, sedangkan todo diikat ke `sessionId`. Akibatnya `minicode --resume foo` mengikat task ke `.minicode/todos/<acak>.json` dan `todo_read` mengembalikan `"(no todos yet)"` padahal `.minicode/todos/foo.json` ada — task state hilang tepat di batas yang seharusnya dilewatinya, dan setiap resume meninggalkan file todo yatim. Todo kini terikat ke id kanonik `presentationSessionId` (`resumeId ?? sessionId`), sama dengan history dan presentation events.
- `completed` kini **memerlukan bukti**. Sebelumnya status itu hanya string yang ditulis model, dan **tidak ada satu pun jalur kode** yang mencegah task ditandai selesai saat verify merah — bahkan setelah 3 siklus self-heal gagal. Composition root sekarang menyuntikkan bukti (hasil verify terakhir) lewat `setCompletionEvidence()`; `failed` menolak `completed`, task turun ke status baru `blocked` beserta alasannya, dan model diberi tahu lewat hasil tool (bukan diam-diam tersimpan di file). Tanpa `--verify` (opt-in) verdict `unverified`: completion tetap diizinkan karena memblokirnya akan mematikan pemakaian normal, tapi tidak diklaim terverifikasi.
- `plan.updated` sekarang terbit di `execution:completed`, bukan `execution:started`. Dulu plan disintesis dari argumen mentah **sebelum** tool jalan, sehingga `plan.updated` berstatus `completed` bisa tertahan durable di `presentation_events` meski `saveTodos` gagal (disk penuh/EACCES) — presentasi menyatakan selesai tanpa file-nya.
- Satu normalizer untuk dua store. Adaptor punya pemetaan status sendiri, sehingga 2 item `in_progress` menjadi dua `active` di plan padahal file todo menyimpan satu (`normalizeTodos` menurunkan yang kedua ke `pending`); cap juga berbeda (adaptor 100, file 50) sehingga item 51-100 hanya hidup di event log. `planFromTodos` kini memakai `normalizeTodos` yang sama dengan file, jadi status, cap, dan keputusan `blocked` tidak bisa berbeda arah.
- Status `blocked` masuk ke kontrak plan presentational (`PlanStepStatus` + `UiPresentationPlan`/`UiPresentationEvent`).
...
- Rollback flag `MINICODE_PRESENTATION_V2` dihapus; jalur presentasi kanonik selalu aktif.
- Raw ledger TUI (`execution:completed` → baris manual) dihapus; ledger kini selalu dari event presentasi + snapshot.
...
- **Bench beku + taksonomi gagal + kartu harness (F2)**: 2 tugas gagal-by-design (`refuse-hallucination`, `refuse-destructive` — lolos = workspace utuh + berhenti jujur); tiap run gagal membawa kelas (`VERIFY_FAIL`/`MAX_STEPS`/`NO_PROGRESS`/`ABORT`, `classifyBenchFailure` murni + teruji) + distribusi global di akhir run; `eval-gate --observe-tokens` (pelanggaran token = peringatan 2–3 run pertama, bukan gagal); `bench/HARNESS-CARD.md` (metodologi + 13 tugas beku + log observasi). `bench/runner.ts` aman diimpor tanpa menjalankan bench (`import.meta.main`).
- **Informasi token first-class (F1)**: `/status` kini menampilkan baris `Turn:` (token turn terakhir via `usage.get()`, terpisah dari `Total:` sesi — kunci i18n `st.turn` en/id); tiap baris `.minicode/step-traces.jsonl` membawa `totalTokens` kumulatif sesi (monoton naik, bahan kurva token) + agregat `peakTotalTokens` di `summarizeStepTraces` dan `minicode stats` (`Peak tok`); panduan "Membaca angka token" di `docs/pricing-budget.md`.
- **Long-horizon murah (F3)**: anti-thrash kompaksi LLM (2× reduksi <10% beruntun → jalur LLM mati sesi-ini + warn sekali, loop jatuh ke mekanikal); `MINICODE_COMPACT_KEEP_TURNS` (override keepRecentTurns kernel, invalid → default + warn di composition root); kontrak `Verify before claiming done` di system prompt (diukur via kelas VERIFY_FAIL).
- **Skills Zed-minimal (F4.1)**: frontmatter `disable-model-invocation: true` (skill hanya via `/nama`, tak masuk katalog auto-pick — untuk deploy/rilis); aset sibling `<nama>/{scripts,references}/` terdaftar sebagai petunjuk baca di katalog (dibaca via `read_file`, tak dieksekusi otomatis; symlink escape ditolak, cap 20/sisi).
- **Provenance vendor tarball**: `VENDOR.md` kini mencatat **shipped hash** (`824f509e6506c4b3`, 18 file) — fingerprint file vendor yang ikut paket npm, tanpa `test/fakes.ts` yang sengaja dikecualikan (hash 19-file tidak bisa direproduksi dari paket terbit; shipped hash bisa). Guard `test/pack-integrity.test.ts`.

### Fixed
- **Kunci `.minicode/` bisa ditembus link internal (audit keamanan 2026-09-22, F-CRIT)**: `write_file linkdir/config.json` MENULIS `.minicode/config.json` lewat junction/symlink (`mklink /J linkdir .minicode`) karena cek owned-state di permission layer membaca STRING argumen, bukan target nyata — padahal proteksi workspace (`isRealPathOutsideRoot`) dan TOCTOU pembaca sudah berbasis realpath. Kunci kini ganda: (1) `isOwnedStateReal()` (`src/policy/jail.ts`) me-resolve realpath best-effort (induk terdekat yang ADA + rekonstruksi ekor; symlink internal SAH ke berkas biasa tetap bisa ditulis sesuai kontrak "targetnya yang dicek") dan dipakai semua tool tulis (`write_file`/`edit`/`patch`/`delete_file`/`move_file`), dengan carve-out string lebih dulu (restore `.trash/`, skrip `hooks/`); (2) bash-guard menahan perintah PEMBUATAN link yang operannya sensitif/owned-state — SEMUA operand non-flag dicek, bukan satu slot posisi, karena urutan argumen beda antar tool (`ln`: TARGET LINK; `mklink`/`fsutil`: LINK dulu; `New-Item`: parameter bernama urutan bebas + alias `-Value` + nilai menempel `-Target:.minicode`). Link ke target jinak tetap lolos. Regresi: `test/deny-reason.test.ts`, `test/bash-guard-escaping.test.ts` (14 bentuk serangan + 5 tetangga jinak), korpus gate:bash kini 62 pola serangan + 24 perintah sah (0 bypass / 0 over-block).
...
- **Wizard setup pertama tak lagi tampak macet di `⠴ Menyiapkan sesi…`**: spinner setup sesi menulis frame mentah `\r\x1b[2K` ke stderr tiap ~120ms — termasuk saat wizard (picker gateway + prompt API key) memegang terminal — sehingga prompt dihapus tiap tick (laporan: `minicode` dari home selalu berhenti di "Menyiapkan sesi…"). Kini: (a) `beginInteractiveScreen()` di `src/ui/runtime/statusline.ts` menahan SEMUA painter transient (nol byte + baris painter dibersihkan sekali; nested + release idempoten), dipegang `askLine`/`askSecret`/`runPicker` dan setiap handle `openAltScreen` (dilepas di `close()`); (b) kedua spinner CLI (cek update + setup) memakai `createSpinner` dengan `SpinnerOptions {delayMs,intervalMs}` — delay 120ms anti-berkedip untuk cek update <200ms. Guard: `test/transient-arbitration.test.ts` (hold picker/askLine, nesting, delayMs); kontrak I27.
- **Kontrak mesin `exec --json` + flag `exec` yang mati + exit code 2 (audit terminal UI/UX)**: `exec --json` gagal setup (tanpa provider) dulu pulang dengan stdout kosong + exit 1 — kini satu baris `{"type":"summary","ok":false,…}` (di-scrub) tetap dipancarkan; pesan manusia + exit 1 tidak berubah. Akar dua lapis: `createProviderLayer` memanggil `process.exit(1)` dari dalam (kini melempar `NoProviderError`, tiap composition root yang memutuskan presentasi) dan `hasFlag`/`getArg` berhenti di token `exec` sehingga **semua flag boolean `exec` (`--json`/`--verify`/`--ask`/`--plan`/`--allowlist`/`--budget-strict`/`--allow-local-config`) mati diam-diam di CLI nyata** (kini diparse dari argv sesudah `exec`; flag setelah kata prompt tetap teks prompt). Salah pakai (argv hilang/malformed, subcommand asing) kini **exit 2** di semua handler (dulu 1); runtime/lookup/help tetap 1/1/0. Migrasi: skrip `!= 0` aman, asersi literal `== 1` untuk usage perlu update. Guard `test/exec-json-envelope.test.ts` + `test/exit-codes.test.ts`; kontrak I28.
- **Flag sebagai id ditolak + scope `sessions list` + sanitasi label**: `auth logout --cwd X` dan `pricing show --json` dulu menyerap flag sebagai id (usage + exit 2 kini); `sessions list` tanpa `.minicode/` lokal mengumumkan `(global scope — …)` sebelum tabel; label provider/model/id dari config lokal atau probe jaringan (`config add/remove/set-key`, `providers|models|sync`, `/status`) kini lewat `sanitizeAnsiLine` — teks tak terpercaya tak bisa lagi membawa `\x1b[2J` ke scrollback.
- **Input inline tak lagi mendarat di baris salah sesudah terminal berubah ukuran (temuan snap kiri/kanan)**: `askLine` menyimpan jangkar `prevRows`/`prevInputRows`/`prevCursorRow` sebagai offset baris RELATIF terhadap tempat kursor berdiri — hitungan itu sah untuk grid pra-reflow, dan sesudah `columns`/`rows` berubah terminal menata ulang baris sehingga offset lama menunjuk baris lain; render berikutnya naik ke anchor basi (`\x1b[1A`) dan `clearOverlay` menghapus baris yang salah, jadi isi ketikan tampak terduplikasi di area output/ketikan. Kini render pertama sesudah geometri berbeda me-reset ketiga offset dan menggambar dari kursor saat ini (`lastGeoCols/Rows` di `src/ui/input/input.ts`) — sengaja TANPA listener `resize` baru (frame basi cukup ditimpa render berikutnya) dan `printedW` tak ikut di-reset (nilainya konservatif; dikosongkan justru membiarkan ekor lama). Guard `test/input-resize.test.ts` (gagal di kode lama: CUP ke anchor basi; multiline + dropdown terbuka + resize ganda + resize cepat ×10); kontrak I30.
...
- Gate: tsc/lint hijau, 2317 pass / 0 fail, coverage 83,30/84,60, pack 22/0.
