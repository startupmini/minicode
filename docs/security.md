...
1. **Bash-guard ternormalisasi** (`src/policy/bash-guard.ts`) — quote pemisah kata dibuang dan assignment sederhana disubstitusi **sebelum** pemeriksaan. Ini menutup kelas bypass, bukan pola individual:
   ...
   | `type .e^nv`, `ty^pe .env` (escape caret cmd.exe) | guard mencocokkan nama LITERAL, sementara cmd.exe melepas `^` di luar kutip ganda sebelum eksekusi — satu caret memindahkan perintah dari "ditahan" ke "lolos" | ditolak: `stripCaretEscapes` berjalan sebelum semua aturan nama/jail (audit 2026-09-20, temuan F1); caret di dalam kutip ganda tidak disentuh (`echo "a^b"` tetap sah). Trade-off jujur: di POSIX caret bukan escape, jadi `cat .e^nv` ikut ditolak (fail-closed; berkas bernama `.e^nv` praktis tidak ada) |
   | `type %USERPROFILE%\notes.txt` (argumen PEMBACA + ekspansi env) | hanya target redirect yang dianggap "tak bisa dipastikan"; argumen pembaca di-`resolve()` sebagai jalur relatif DI DALAM cwd | ditolak: aturan `%NAMA%` kini simetris untuk argumen pembaca (temuan F2). `%` tunggal (`100%.txt`) tetap lolos |
   | `for /f %i in (.env) do @echo %i` (pembaca bawaan cmd.exe) | `for` adalah loop, bukan utilitas → tidak ada di `READERS`, sehingga target berkasnya tak diperiksa sama sekali | ditolak: set berkas `for /f … in (…)` diekstrak (`forFileSets`) dan diperiksa seperti argumen pembaca (temuan F3) |
