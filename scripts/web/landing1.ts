...
<div class="term" id="install"><div class="term-cap"><span class="material-symbols-outlined" style="font-size:13px">terminal</span>instalasi — 2 langkah<span class="sp"><button class="copybtn" data-copy="${installRaw}" aria-label="Salin perintah instalasi"><span class="material-symbols-outlined">content_copy</span></button></span></div><pre><code>${install}</code></pre></div>
...
export function landingHow(): string {
  ...
  return `<section id="cara-kerja"><h2>Cara kerja.</h2><p class="sub">Lima langkah yang sama setiap kali, tanpa kejutan.</p><div class="feat-list">${steps
    .map(([h, p]) => `<div class="feat"><h3>${h}</h3><p>${p}</p></div>`)
    .join("")}</div></section>`
}
